return function(config)
    ...
    for k, v in pairs(defaultConfig) do
        finalConfig[k] = (config and config[k] ~= nil) and config[k] or v
    end
    ...
    container.Position = finalConfig.Position
    container.Size = UDim2.new(0, finalConfig.Width, 0, 0)
    ...
    local function CreateNotification(params)
        ...
        local colorScheme = NotificationService.Colors[notifyType] or NotificationService.Colors.DEFAULT
        
        -- Limit Notifications
        if #activeNotifications >= finalConfig.MaxNotifications then
            table.remove(activeNotifications, 1):Destroy()
        end
        
        -- Create Notification Frame
        local frame = Instance.new("Frame")
        frame.Size = UDim2.new(1, 0, 0, finalConfig.NotificationHeight)
        ...
        -- Positioning
        local yOffset = (#activeNotifications * (finalConfig.NotificationHeight + finalConfig.Spacing))
                ...
                tweenOut.Completed:Connect(function()
                    for i, notification in ipairs(activeNotifications) do
                        local newPos = TweenService:Create(notification, 
                            ...
                            {Position = UDim2.new(0, 0, 0, (i-1) * (finalConfig.NotificationHeight + finalConfig.Spacing))}
                        )
                        newPos:Play()
                    end
    ...
    -- Return functions
    return CreateNotification, ClearNotifications
end